...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
   $cb = $_POST['cb'] ?? [];
   $count = count($cb);
     header('Location: http://localhost/capybaros/homework/webMechanika/vm9.php?count=' . $count);
    die;
 }


if (isset($_GET['count'])) {
     $count = $_GET['count'];     $color = 'white';
 } else {
    $color = 'black';
    $letters = range('A', 'J');
...
<?php if ($color == 'white'): ?>                 
    ...
    <a href="http://localhost/capybaros/homework/webMechanika/vm9.php">BACK</a>
